...
const { addUserToBlacklist, isUserBlacklisted } = require('../../db/managedb');

module.exports = {
    data: new SlashCommandBuilder()
        .setName('admin_adduser')
        .setDescription('ブラックリストにユーザーを追加します。')
        .addStringOption(option => 
            option.setName('ユーザーid')
                .setDescription('ブラックリストに追加するユーザーのID')
                .setRequired(true))
        .addStringOption(option => 
            option.setName('理由')
        ...
        try {
            ...
            if (isBlacklisted) {
                const embed = new EmbedBuilder()
                    ...
                    .setDescription(`ユーザーID：${userId}は既にブラックリストに登録されています。`);
                
                await interaction.reply({ embeds: [embed] });
                return;
            }

            await addUserToBlacklist(userId, reason);
            ...
            const embed = new EmbedBuilder()
                ...
                .setDescription(`ユーザーの追加中にエラーが発生しました：${err.message}`);
